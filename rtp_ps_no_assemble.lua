-- rtp_ps_no_assemble.lua
-- PS dissector: dissects each RTP payload individually WITHOUT fragment reassembly.
-- More robust under packet reordering; may show incomplete trees for split frames.
--
-- ⚠️  Cannot be used simultaneously with rtp_ps_assemble.lua —
...
do
    ...
    -- -----------------------------------------------------------------------
    -- Lookup tables  (identical to assemble variant — both are self-contained)
    -- -----------------------------------------------------------------------
    ...
    -- -----------------------------------------------------------------------
    -- Parse helpers  (same logic as assemble variant)
    -- -----------------------------------------------------------------------
    local function dis_pack_header(tvb,tree,off)
        ...
        t:add(F.mux_rate,tvb:range(off+10,3)); t:add(F.stuffing_len,tvb:range(off+13,1))
    ...
    local function dis_system_header(tvb,tree,off)
        ...
        t:add(F.sys_start,tvb:range(off,4));  t:add(F.sys_len,tvb:range(off+4,2))
        ...
        if hlen>6 then
            ...
            while r>=3 do
                ...
                t:add(F.sys_std_scale,tvb:range(p+1,2)); t:add(F.sys_std_bound,tvb:range(p+1,2))
                p=p+3; r=r-3
            end
        end
        return hlen+6
    ...
    local function dis_stream_map(tvb,tree,off,sid_map)
        ...
        while r>=4 do
            ...
            t:add(F.psm_es_type,tvb:range(p,1)); t:add(F.psm_es_id,tvb:range(p+1,1))
    ...
    -- -----------------------------------------------------------------------
    -- Dissector  (no-assemble: process each RTP payload directly)
    -- -----------------------------------------------------------------------
    function proto_ps.dissector(tvb,pinfo,tree)
        ...
        local pt=tree:add(proto_ps,tvb:range()); local off=0
        if is_ps_header(tvb,off) then
            off=off+dis_pack_header(tvb,pt,off)
            if is_system_header(tvb,off) then
                off=off+dis_system_header(tvb,pt,off)
                if off+4<=tvb:len() and tvb:range(off+3,1):uint()==0xbc then
                    off=off+dis_stream_map(tvb,pt,off,stream_id_map)
                end
            end
        end
        while is_pes_header(tvb,off) do
            local step=dis_pes(tvb,pt,off,pinfo,stream_id_map,h264_dis,h265_dis,pcma_dis,pcmu_dis)
            if not step or step==0 then break end; off=off+step
        end
        if off<tvb:len() and not is_pes_header(tvb,off) then
            pt:add(F.raw_data,tvb:range(off)):set_text(
                string.format("Continuation data (%d bytes)",tvb:len()-off))
        end
    end
    ...
    function proto_ps.init()
        stream_id_map={}; sub_dis_loaded=false
